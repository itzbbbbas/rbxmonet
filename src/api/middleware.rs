...
#[derive(Clone, Debug)]
pub struct RobloxXsrfMiddleware {
    token: Arc<Mutex<Option<String>>>,
}

impl RobloxXsrfMiddleware {
    pub fn new() -> Self {
        Self {
...
impl Middleware for RobloxXsrfMiddleware {
    async fn handle(
        &self,
        req: Request,
        ...
        // Attach cached XSRF token if we have one.
        if let Some(token) = self.token.lock().await.clone() {
            if let Ok(value) = HeaderValue::from_str(&token) {
                req.headers_mut().insert("x-csrf-token", value);
            }
        }

        // Try once, refresh token + retry once on 403 XSRF.
        for attempt in 0..2 {
            ...
            *self.token.lock().await = Some(new_token.clone());
            ...
            let Some(mut cloned) = req_clone else {
                return Ok(resp);
            };

            if let Ok(value) = HeaderValue::from_str(&new_token) {
                cloned.headers_mut().insert("x-csrf-token", value);
            }
            req = cloned;
        }
