...
nest! {
    ...
    pub struct VCSProducts {
        ...
        pub passes: HashMap<String, pub struct Product {
            ...
            #[serde(default, alias = "icon-id")]
            pub icon_id: Option<u64>,
            #[serde(default, alias = "icon-hash")]
        ...
        // endpoint for SubscriptionProduct definitions today, so this map
        // is HAND-MAINTAINED. `sync` never iterates it, `download` never
        // mutates it — the pipeline only round-trips entries through TOML
        // and emits them into the Luau codegen output. Run `regen-luau`
        // after hand-edits to refresh monets.luau.
        #[serde(default)]
        pub subscriptions: HashMap<String, pub struct Subscription {
            pub id: Option<u64>,
            pub name: String,
            #[serde(default)]
            pub description: Option<String>,
            ...
            #[serde(default)]
            pub path: Option<String>,
            #[serde(default, alias = "basic-price-tier")]
            pub basic_price_tier: Option<String>,
            #[serde(default)]
...
const LOCKFILE_PATH: &str = "rbxmonet.lock.toml";

#[derive(Default, Debug, Clone, Serialize, Deserialize)]
pub struct IconLockEntry {
...
pub struct IconLock {
    ...
    pub passes: HashMap<String, IconLockEntry>,
    #[serde(default, skip_serializing_if = "HashMap::is_empty")]
    pub products: HashMap<String, IconLockEntry>,
    #[serde(default, skip_serializing_if = "HashMap::is_empty")]
    pub badges: HashMap<String, IconLockEntry>,
}

fn apply_lock(map: &mut HashMap<String, Product>, lock: &HashMap<String, IconLockEntry>) {
    for (slug, p) in map.iter_mut() {
        if let Some(entry) = lock.get(slug) {
            if entry.icon_id.is_some() {
                p.icon_id = entry.icon_id;
            }
            if entry.icon_hash.is_some() {
                p.icon_hash = entry.icon_hash.clone();
            }
        }
    }
}

fn collect_lock(map: &HashMap<String, Product>) -> HashMap<String, IconLockEntry> {
    map.iter()
        .filter_map(|(slug, p)| {
            if p.icon_id.is_some() || p.icon_hash.is_some() {
                Some((
                    slug.clone(),
                    IconLockEntry {
                        icon_id: p.icon_id,
                        icon_hash: p.icon_hash.clone(),
                    },
                ))
            } else {
                None
...
impl VCSProducts {
    pub async fn get_products() -> Result<Self> {
        ...
        normalize_slug_keys(&mut products.subscriptions);

        // Lockfile load (new in 0.1.35). When present, its entries take
        // precedence over any icon_id / icon_hash that older versions wrote
        // into rbxmonet.toml — `save_products` will strip those on next save.
        if let Ok(lock_data) = fs::read(LOCKFILE_PATH).await
            && let Ok(lock_text) = std::str::from_utf8(&lock_data)
            && let Ok(lock) = toml::from_str::<IconLock>(lock_text)
        {
            apply_lock(&mut products.passes, &lock.passes);
            apply_lock(&mut products.products, &lock.products);
            apply_lock(&mut products.badges, &lock.badges);
        }

        Ok(products)
    }

    pub async fn save_lock(&self) -> Result<()> {
        let lock = IconLock {
            passes: collect_lock(&self.passes),
            products: collect_lock(&self.products),
            badges: collect_lock(&self.badges),
        };

        if lock.passes.is_empty() && lock.products.is_empty() && lock.badges.is_empty() {
            // Nothing to track. Remove a stale lockfile if one exists so the
            // working tree doesn't carry empty-state cruft.
            let _ = fs::remove_file(LOCKFILE_PATH).await;
            return Ok(());
        }

        let body = toml::to_string_pretty(&lock).map_err(|e| format!("serialize lock: {e}"))?;
        let header = "# Auto-generated by rbxmonet. Do not hand-edit.\n# Tracks icon asset ids + on-disk BLAKE3 hashes so `sync` can skip unchanged icons.\n\n";
        let mut file = fs::File::create(LOCKFILE_PATH).await?;
    ...
    pub async fn save_products(&self) -> Result<()> {
        ...
        subscriptions.retain(|k, _| self.subscriptions.contains_key(k));

        let strip_lock_keys = |item: &mut toml_edit::Item| {
        ...
        for pass in &self.passes {
            let mut item: toml_edit::Item = pass.1.into();
            strip_lock_keys(&mut item);
            passes[&pass.0] = item;
        }
        for product in &self.products {
            let mut item: toml_edit::Item = product.1.into();
            strip_lock_keys(&mut item);
            products[&product.0] = item;
        }
        ...
        for subscription in &self.subscriptions {
            let mut item: toml_edit::Item = subscription.1.into();
            strip_lock_keys(&mut item);
            subscriptions[&subscription.0] = item;
        }
    ...
    pub async fn serialize_luau(&self) -> Result<()> {
        ...
        let subscription_path = resolve_section("subscriptions", "subscriptions");
        ...
        for (slug, s) in &self.subscriptions {
            let leaf = LeafKind::Subscription {
                id: s.id.unwrap_or(0),
                name: s.name.clone(),
                description: s.description.clone().unwrap_or_default(),
                icon: s.icon_id,
                period: s.period.clone(),
                basic_price_tier: s.basic_price_tier.clone(),
            };
            insert_into_tree(&mut tree, &split_path(&subscription_path), slug, leaf);
        }
        ...
        contents += "export type Kind = \"Pass\" | \"Product\" | \"Badge\" | \"Subscription\"\n";
        contents += "export type Pass = { id: number, price: number, kind: Kind, name: string, description: string, icon: string? }\n";
        contents += "export type Product = { id: number, price: number, kind: Kind, name: string, description: string, icon: string? }\n";
        contents += "export type Badge = { id: number, kind: Kind, name: string, description: string, icon: string? }\n";
        contents += "export type Subscription = { id: number, kind: Kind, name: string, description: string, period: string?, basicPriceTier: string?, icon: string? }\n\n";
        ...
        if self.codegen.typescript {
            ...
            dts += "export type Kind = \"Pass\" | \"Product\" | \"Badge\" | \"Subscription\";\n";
            dts += "export interface Pass { id: number; price: number; kind: \"Pass\"; name: string; description: string; icon?: string }\n";
            dts += "export interface Product { id: number; price: number; kind: \"Product\"; name: string; description: string; icon?: string }\n";
            dts += "export interface Badge { id: number; kind: \"Badge\"; name: string; description: string; icon?: string }\n";
            dts += "export interface Subscription { id: number; kind: \"Subscription\"; name: string; description: string; period?: string; basicPriceTier?: string; icon?: string }\n\n";
            match self.codegen.style {
...
fn render_leaf(out: &mut String, leaf: &LeafKind) {
    match leaf {
        LeafKind::Rich {
            ...
            let icon_suffix = match icon {
                Some(asset_id) => format!(", icon = \"rbxassetid://{}\"", asset_id),
                None => String::new(),
            };
            match section {
                SectionKind::Badge => {
        ...
        LeafKind::Subscription {
            ...
            out.push_str(&format_subscription_leaf(
                *id,
                name,
                description,
                *icon,
                period.as_deref(),
                basic_price_tier.as_deref(),
            ));
        }
        LeafKind::IdOnly(id) => {
...
fn format_subscription_leaf(
    id: u64,
    name: &str,
    description: &str,
    icon: Option<u64>,
    period: Option<&str>,
    basic_price_tier: Option<&str>,
) -> String {
    let mut extra = String::new();
    if let Some(p) = period {
        extra.push_str(&format!(", period = {:?}", p));
    }
    if let Some(t) = basic_price_tier {
        extra.push_str(&format!(", basicPriceTier = {:?}", t));
    }
    if let Some(asset_id) = icon {
        extra.push_str(&format!(", icon = \"rbxassetid://{}\"", asset_id));
    }
    format!(
        "{{ id = {}, kind = \"Subscription\" :: Kind, name = {:?}, description = {:?}{} }}",
...
fn leaf_ts_type(leaf: &LeafKind) -> &'static str {
    match leaf {
        ...
        LeafKind::Subscription { .. } => "Subscription",
    }
}
...
enum LeafKind {
    ...
    Subscription {
        ...
        basic_price_tier: Option<String>,
    },
    IdOnly(u64),
}

fn normalize_slug_keys<V>(map: &mut HashMap<String, V>) {
...
fn render_node(out: &mut String, node: &CodegenNode, depth: usize) {
    ...
    match node {
        ...
        CodegenNode::Leaf(LeafKind::Subscription {
            id,
            name,
            description,
            icon,
...
impl From<&Subscription> for toml_edit::Item {
    fn from(sub: &Subscription) -> Self {
        ...
        if let Some(period) = &sub.period {
            table["period"] = toml_edit::value(period.clone());
        }
        if let Some(tier) = &sub.basic_price_tier {
            table["basic_price_tier"] = toml_edit::value(tier.clone());
        }
        if let Some(icon) = &sub.icon {
            table["icon"] = toml_edit::value(icon.clone());
        }
        // icon_id / icon_hash live in rbxmonet.lock.toml — never written here.
...
impl From<&Product> for toml_edit::Item {
    fn from(prod: &Product) -> Self {
        ...
        // icon_id and icon_hash live in rbxmonet.lock.toml — never written to the main TOML.
