...
impl VCSProducts {
    ...
    pub async fn serialize_luau(&self) -> Result<()> {
        ...
        contents += "export type Pass = { id: number, price: number, name: string, description: string }\n";
        ...
        if self.codegen.typescript {
            ...
            dts += "export interface Pass { id: number; price: number; name: string; description: string }\n";
